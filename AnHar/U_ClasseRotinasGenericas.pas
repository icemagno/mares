...
{Procedimento que gera o arquivo de Alturas Horárias padrão para a DLL das rotinas Alpha.
OBS: O código da Estação Maregráfica deve estar previamente cadastrado e as datas passadas
por parâmetros (DataIni, DataFim) devem estar no formato 'dd/mm/yyyy'. O arquivo será
gerado no caminho especificado na variável 'Arquivo'. Se o arquivo já existe ele será
substituido.}
var
...
function TClasseRotinasGenericas.ValidaPeriodoAlturasTempDll(Conexao: TADOConnection; CodEstacao, CodEquipEstacao: PeqInteiroSs;
  DataIni, DataFim: string): Boolean;
{Função para validar um período de Alturas Horárias. Se o período informado estiver
cadastrado dia a dia sem quebras (dias pertencentes ao período que não estão cadastrados)
no Banco de Dados a função retorna true. As datas devem estar no formato dd/mm/yyyy.}
var
  AltHor: TADOQuery;
...
procedure TClasseRotinasGenericas.GeraArquivoAlturasEquipamento(Conexao: TADOConnection; CodEstacao, CodEquipEstacao: PeqInteiroSs; DataIni,
  DataFim, Arquivo: string; Filtro: MinInteiroSs);
{Procedimento que gera um arquivo texto do Período de Alturas Horárias informado.
As variáveis DataIni e DataFim devem estar no formato 'dd/mm/yyyy'.}
var
...
begin
  ...
  AssignFile(ArqSaida, Arquivo);
  Rewrite(ArqSaida);
  Writeln(ArqSaida, 'PERÍODO DE ALTURAS OBSERVADAS');
  Writeln(ArqSaida, 'ESTAÇÃO:' + FormatFloat('00000', EstMaregr.FieldByName('num_estacao_maregrafica').AsInteger));
  Writeln(ArqSaida, 'NOME ESTAÇÃO:' + EstMaregr.FieldByName('nome_estacao_maregrafica').AsString);
  ...
  Writeln(ArqSaida, 'ALTURAS EM CENTÍMETROS');
  Writeln(ArqSaida, 'HORÁRIO PAPA');
  Writeln(ArqSaida, '#');
  EstMaregr.Close;
  AltHor.SQL.Clear;
...
begin
  ...
  writeln('Codigo da analise: ' + AnaliseMares.FieldByName('cod_analise_mares').AsString );
...
{Função para verificar se existem Alturas Horárias cadastradas dentro do período informado.
Caso existam a função retorna true.}
var
  AltHor: TADOQuery;
begin
  AltHor:= TADOQuery.Create(nil);
...
procedure TClasseRotinasGenericas.GeraArquivoAlturasTempEquipamento(Conexao: TADOConnection; CodEstacao, CodEquipEstacao: PeqInteiroSs; DataInicial,
  DataFinal: TDate; Arquivo: string; Filtro: Boolean);
{Procedimento que gera um arquivo texto do Período de Alturas Horárias informado.
As variáveis DataInicial e DataFinal devem estar no formato 'dd/mm/yyyy'.}
var
  EstMaregr, AltHor: TADOQuery;
  ArqSaida: TextFile;
  TaxaAmostragem: MinInteiroSs;
...
{Data: 08/09/2009
Objetivo: Função para retornar a diferença entre períodos DataInicial deve ser sempre menor do que DataFinal.
Tipo do retorno:
1 - em minutos;
2 - em horas;
3 - em dias.
...
{Data: 08/09/2009
Objetivo: Interpolar um conjunto de dados observados em uma taxa para outra (máximo 20 dias minuto a minuto).
Rotina original SPLINE20 (Prof. Geraldo).
Parâmetros: ITempo - taxa dos dados de entrada; DeltaT: taxa dos dados de saída.}
var
...
{Função que retorna o conteúdo de uma string após o separador "sep".}
var
...
{Função que verifica se uma data é realmente o dia posterior à outra.}
begin
...
{Função que valida uma data. O parâmetro 'Data' para ser validado
...
{Função que preenche com espaços em branco à direita ou à esquerda de uma string.
...
{Função que valida uma data-hora.}
begin
  try
